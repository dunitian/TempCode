using System;

namespace Snowflake.Net
{
    /// <summary>
    /// 自定义 ID 生成器
    /// ID 生成规则: ID长达 64 bits
    /// | 41 bits: Timestamp (毫秒) | 3 bits: 区域（机房） | 10 bits: 机器编号 | 10 bits: 序列号 |
    /// </summary>
    public class Snowflake
    {
        //基准时间
        public const long Twepoch = 1288834974657L; //Thu, 04 Nov 2010 01:42:54 GMT
                                                    //机器标识位数
        const int WorkerIdBits = 10;
        //区域标志位数
        const int RegionIdBits = 3;
        //序列号识位数
        const int SequenceBits = 10;
        //机器ID最大值
        const long MaxWorkerId = -1L ^ (-1L << WorkerIdBits);
        //区域标志ID最大值
        const long MaxRegionId = -1L ^ (-1L << RegionIdBits);
        //序列号ID最大值
        private const long SequenceMask = -1L ^ (-1L << SequenceBits);

        //机器ID偏左移10位
        private const int WorkerIdShift = SequenceBits;
        //业务ID偏左移20位
        private const int RegionIdShift = SequenceBits + WorkerIdBits;
        //时间毫秒左移23位
        public const int TimestampLeftShift = SequenceBits + WorkerIdBits + RegionIdBits;


        private long _sequence = 0L;
        private long _lastTimestamp = -1L;

        public long WorkerId { get; protected set; }
        public long RegionId { get; protected set; }
        public long Sequence
        {
            get { return _sequence; }
            internal set { _sequence = value; }
        }

        public static Snowflake snowflake = new Snowflake(1, 1);
        /// <summary>
        /// 初始化
        /// </summary>
        /// <returns></returns>
        public static Snowflake Init()
        {
            if (snowflake == null)
                snowflake = new Snowflake(1, 1);
            return snowflake;
        }
        /// <summary>
        /// 带参构造函数
        /// </summary>
        public Snowflake(long workerId, long regionId, long sequence = 0L)
        {

            // 如果超出范围就抛出异常
            if (workerId > MaxWorkerId || workerId < 0)
            {
                throw new ArgumentException(string.Format("worker Id 必须大于0，且不能大于MaxWorkerId： {0}", MaxWorkerId));
            }

            if (regionId > MaxRegionId || regionId < 0)
            {
                throw new ArgumentException(string.Format("region Id 必须大于0，且不能大于MaxWorkerId： {0}", MaxRegionId));
            }

            WorkerId = workerId;
            RegionId = regionId;
            _sequence = sequence;
        }



        // def get_timestamp() = System.currentTimeMillis

        readonly object _lock = new object();

        public virtual long GetId()
        {
            lock (_lock)
            {
                var timestamp = TimeGen();

                if (timestamp < _lastTimestamp)
                {
                    throw new Exception(string.Format("时间戳必须大于上一次生成ID的时间戳.  拒绝为{0}毫秒生成id", _lastTimestamp - timestamp));
                }

                //如果上次生成时间和当前时间相同,在同一毫秒内
                if (_lastTimestamp == timestamp)
                {
                    //sequence自增，因为sequence只有10bit，所以和sequenceMask相与一下，去掉高位
                    _sequence = (_sequence + 1) & SequenceMask;
                    //判断是否溢出,也就是每毫秒内超过1024，当为1024时，与sequenceMask相与，sequence就等于0
                    if (_sequence == 0)
                    {
                        //自旋等待到下一毫秒
                        timestamp = TilNextMillis(_lastTimestamp);
                    }
                }
                else
                {
                    // 如果和上次生成时间不同,重置sequence，就是下一毫秒开始，sequence计数重新从0开始累加,
                    // 为了保证尾数随机性更大一些,最后一位设置一个随机数
                    _sequence = new Random().Next(10);//0;
                }

                _lastTimestamp = timestamp;
                return ((timestamp - Twepoch) << TimestampLeftShift) | (RegionId << RegionIdShift) | (WorkerId << WorkerIdShift) | _sequence;
            }
        }

        // 防止产生的时间比之前的时间还要小（由于NTP回拨等问题）,保持增量的趋势.
        protected virtual long TilNextMillis(long lastTimestamp)
        {
            var timestamp = TimeGen();
            while (timestamp <= lastTimestamp)
            {
                timestamp = TimeGen();
            }
            return timestamp;
        }

        // 获取当前的时间戳
        protected virtual long TimeGen()
        {
            return TimeExtensions.CurrentTimeMillis();
        }
    }
}
