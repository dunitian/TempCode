...
namespace _07.TaskDContinueWith
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //任务执行完毕后执行
            task.ContinueWith(t => Console.WriteLine("计算之后：{0}。线程ID：{1}\n", t.Result, t.Id), TaskContinuationOptions.OnlyOnRanToCompletion);
            //发生异常的时候执行
            task.ContinueWith(t => Console.WriteLine("异常信息：{0}。线程ID：{1}\n", t.Exception, t.Id), TaskContinuationOptions.OnlyOnFaulted);
            //任务取消的时候执行
            task.ContinueWith(t => Console.WriteLine("任务已经取消，线程ID：{0}\n", t.Id),TaskContinuationOptions.OnlyOnCanceled);
